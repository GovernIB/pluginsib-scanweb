 ...
 * @author anadal
 */
public class DigitalIBMassive2ScanWebPlugin extends AbstractScanWebPlugin {
    ...
    *
    */
    public DigitalIBMassive2ScanWebPlugin() {
        super();
    }
     ...
     * @param properties
     */
    public DigitalIBMassive2ScanWebPlugin(String propertyKeyBase, Properties properties) {
     ...
     * @param propertyKeyBase
     */
    public DigitalIBMassive2ScanWebPlugin(String propertyKeyBase) {
    ...
    public Set<String> getSupportedFlagsByScanType(String scanType) {
        ...
        if (ScanWebDocument.SCANTYPE_MIME_PDF.equals(scanType)) {
            ...
            try {
                ...
                ApiMassiveScanWebSimpleApi api = getApiMassiveScanWebSimple();
                ...
                for (MassiveScanWebSimpleAvailableProfile profile : profiles.getAvailableProfiles()) {
                    ...
                    if (CONSTANTS.getMassiveScanWebSimpleAvailableProfilePROFILETYPEONLYSCAN()
                            .equals(profile.getProfileType())) {

                        // PROFILETYPEONLYSCAN:
                        flags.add(ScanWebDocument.FLAG_PLAIN);
                    } else if (CONSTANTS.getMassiveScanWebSimpleAvailableProfilePROFILETYPESCANANDSIGNATUREANDCUSTODY()
                            .equals(profile.getProfileType())) {
                        log.error(
                                "Tipus de Perfil amb CUSTODY no es suportat per ScanWebAPI. Només s'obtindrà informació de la firma.",
                                new Exception());

                    } else if (CONSTANTS.getMassiveScanWebSimpleAvailableProfilePROFILETYPESCANANDSIGNATURE()
                            .equals(profile.getProfileType())) {
    ...
    */
    ...
    protected void postSelectedProfile(String absolutePluginRequestPath, String relativePluginRequestPath,
        ...
        try {
            ...
            profileRequest.setProfileCode(profileCode);
            profileRequest.setLocale(languageUI);
...

            {
                ...
                if (CONSTANTS.getMassiveScanWebSimpleAvailableProfilePROFILETYPEONLYSCAN()
                        .equals(scanWebProfileSelected.getProfileType())) {
                    ...
                    transacctionIdRequest.setTransactionName(scanWebRequest.getTransactionName());
                    ...
                    transacctionIdRequest.setFuncionariUsername(funcionariUsername);
                        ...
                        .equals(scanWebProfileSelected.getProfileType())
                        || CONSTANTS.getMassiveScanWebSimpleAvailableProfilePROFILETYPESCANANDSIGNATURE()
                                .equals(scanWebProfileSelected.getProfileType())) {
                    MassiveScanWebSimpleSignatureParameters signatureParameters = getSignatureParameters(scanWebRequest,
                            locale);
            ...
            startTransactionInfo.setTransactionID(digitalIBTransactionID);
    ...
    protected void finalPage(String absolutePluginRequestPath, String relativePluginRequestPath, String scanWebID,
        ...
        try {
            ...
            if (localStatus.getStatus() != ScanWebStatus.STATUS_IN_PROGRESS) {
                ...
                if (CONSTANTS.getMassiveScanWebSimpleStatusSTATUSFINALOK().equals(transResult.getStatus())) {
                    ...
                    for (String sub : subtransaccions) {
                        ...
                        Integer statusID = subtransactionStatus.getStatus();
                        ...
                        if (CONSTANTS.getMassiveScanWebSimpleStatusSTATUSREQUESTEDID().equals(statusID)) { // = 0;
                            status.setStatus(ScanWebStatus.STATUS_FINAL_ERROR);
                        ...
                        } else if (CONSTANTS.getMassiveScanWebSimpleStatusSTATUSFINALERROR().equals(statusID)) {// = -1;
                        ...
                        } else if (CONSTANTS.getMassiveScanWebSimpleStatusSTATUSCANCELLED().equals(statusID)) { // = -2;
                        ...
                        } else if (CONSTANTS.getMassiveScanWebSimpleStatusSTATUSINPROGRESS().equals(statusID)) { // = 1;
                    ...
                    if (status.getStatus() != ScanWebStatus.STATUS_FINAL_ERROR
                        ...
                        for (String subID : subtransaccions) {
                            ...
                            srr = new MassiveScanWebSimpleSubtransactionResultRequest();

                            srr.setReturnScannedFile(true);
                            srr.setReturnSignedFile(true);
                            srr.setSubtransactionID(subID);
    ...
    protected ScanWebDocument processarSubtransactionScan(MassiveScanWebSimpleSubtransactionResult result) {
        ...
        if (result.getSignedFile() != null) {
            ...
            if (sssfi != null) {
                ...
                if (sssfi.getValidationInfo() != null) {
                    ...
                    validationInfo = new ScanWebResultSignValidationInfo(vi.isCheckAdministrationIDOfSigner(),
                            vi.isCheckDocumentModifications(), vi.isCheckValidationSignature());
                }
                ...
                signInfo = new ScanWebResultSignInfo(sssfi.getSignOperation(), sssfi.getSignType(),
                        ...
                        sssfi.isTimeStampIncluded(), sssfi.isPolicyIncluded(), sssfi.getEniTipoFirma(),
        ...
        final Date scanDate;
        if (scannedInfo.getScanDate() == null) {
            scanDate = null;
        } else {
            scanDate = new Date(scannedInfo.getScanDate().toInstant().toEpochMilli());
        }
        ...
        final Boolean ocr = scannedInfo.isOcr();

        final Boolean duplex = scannedInfo.isDuplex();
    ...
    protected void processarMetadadaEstatElaboracio(List<Metadata> metadatas,
        ...
        if (arxiuRequired.getDocumentElaborationState() != null) {
            ...
            if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTELABORATIONSTATEORIGINAL()
                    .equals(des)) {
            ...
            } else if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTELABORATIONSTATECOPIACF()
                    .equals(des)) {
                addMetadata(metadatas, MetadataConstants.ENI_ESTADO_ELABORACION,
                        MetadataConstants.EstadoElaboracionConstants.ESTADO_ELABORACION_COPIA_CF);
            } else if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTELABORATIONSTATECOPIADP()
                    .equals(des)) {
                addMetadata(metadatas, MetadataConstants.ENI_ESTADO_ELABORACION,
                        MetadataConstants.EstadoElaboracionConstants.ESTADO_ELABORACION_COPIA_DP);
            } else if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTELABORATIONSTATECOPIAPR()
                    .equals(des)) {
                addMetadata(metadatas, MetadataConstants.ENI_ESTADO_ELABORACION,
    ...
    protected void processarMetadadaOrigen(List<Metadata> metadatas,
        ...
        if (arxiuRequired.getDocumentOrigen() != null) {
            ...
            if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTORIGENADMINISTRACIO()
                    .equals(arxiuRequired.getDocumentOrigen())) {
                addMetadata(metadatas, MetadataConstants.ENI_ORIGEN, MetadataConstants.OrigenConstants.ADMINISTRACION);
            } else if (CONSTANTS.getMassiveScanWebSimpleArxiuRequiredParametersDOCUMENTORIGENCIUTADA()
                    .equals(arxiuRequired.getDocumentOrigen())) {
    ...
    protected void processarMetadadaProfundadadColor(List<Metadata> metadatas,
        ...
        if (scannedFileInfo.getPixelType() == null) {
            ...
            if (CONSTANTS.getMassiveScanWebSimpleScannedFileInfoPIXELTYPEBLACKWHITE()
                    .equals(scannedFileInfo.getPixelType())) {
                profundidadDeColor = MetadataConstants.ProfundidadColorConstants.BW;
                // XYZ ZZZ TRA
                pixelType = "Blanc i negre";
            } else if (CONSTANTS.getMassiveScanWebSimpleScannedFileInfoPIXELTYPEGRAY()
                    .equals(scannedFileInfo.getPixelType())) {
                profundidadDeColor = MetadataConstants.ProfundidadColorConstants.GRAY;
                // XYZ ZZZ TRA
                pixelType = "Escala de grisos";
            } else if (CONSTANTS.getMassiveScanWebSimpleScannedFileInfoPIXELTYPECOLOR()
                    .equals(scannedFileInfo.getPixelType())) {
    ...
    private MassiveScanWebSimpleSignatureParameters getSignatureParameters(ScanWebRequest scanWebRequest, Locale locale)
        ...
        signatureParameters.setFunctionaryFullName(swsi.getFunctionaryFullName());
        signatureParameters.setFunctionaryAdministrationID(swsi.getFunctionaryAdministrationID());
        signatureParameters.setFunctionayDIR3Unit(swsi.getFunctionaryUnitDIR3());
