...
public class ScanWebPluginManager {
    ...
    public static List<Plugin> getAllPlugins() {
        if (plugins == null) {
            ...
            if (pluginPropertiesPath == null) {
                log.error("La propietat 'scanwebplugins.path' " + "no te valor assignat.", new Exception());
                return null;
            }
            ...
            try {
                ...
                log.error(
                        "Error desconegut llegint les propietats del fitxer " + pluginPropertiesFile.getAbsolutePath(),
                        new Exception());
                return null;
            }
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                for (Object object : keys) {
                    ...
                    if (key.startsWith(base)) {
                        ...
                        log.info("[" + PROPERTIES_BASE + nomFinal  +"] => " +value);
    ...
    protected static IScanWebPlugin getInstanceByPlugin(Plugin plugin) throws Exception {
        ...
        log.info("\n\n\n getInstanceByPlugin => NOM " + plugin.getNom() + ")\n\n\n");
        
        log.info("\n\n\n getInstanceByPlugin => BASE " + PROPERTIES_BASE + ")\n\n\n");
        
        log.info("\n\n\n getInstanceByPlugin => PROPERTIES " + prop + ")\n\n\n");
        
        Class<?> cls = Class.forName(plugin.getClasse().trim());
        
        log.info("\n\n\n getInstanceByPlugin => CLASSE " + cls + ")\n\n\n");

        instance = (IScanWebPlugin) PluginsManager.instancePluginByClass(cls, PROPERTIES_BASE, prop);
