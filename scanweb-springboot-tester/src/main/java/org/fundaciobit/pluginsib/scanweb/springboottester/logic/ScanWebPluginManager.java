...
public class ScanWebPluginManager {
    ...
    private static File pluginsFile = null;

    public static void setPluginsFile(File pf) {
        pluginsFile = pf;
    }
    ...
    public static List<Plugin> getAllPlugins() {
        if (plugins == null) {
            ...
            if (pluginsFile == null) {
                log.error(
                        "No s'ha cridat al mètode ScanWebPluginManager.setPluginsFile(..) per inicialitzar el fitxer de configuració de plugins.",
                        new Exception());
                return null;
            }

            if (!pluginsFile.exists()) {
                log.error("No existeix el fitxer  " + pluginsFile.getAbsolutePath(), new Exception());
                return null;
            }
            ...
            try {
                ...
                Reader reader = new InputStreamReader(new FileInputStream(pluginsFile), "UTF-8");
                pluginProperties.load(reader);

            } catch (Exception e) {
                log.error("Error desconegut llegint les propietats del fitxer " + pluginsFile.getAbsolutePath(),
                        new Exception());
                return null;
            }
    ...
    protected static IScanWebPlugin getInstanceByPlugin(Plugin plugin) throws Exception {
        ...
        instance = (IScanWebPlugin) PluginsManager.instancePluginByClassName(plugin.getClasse(), PROPERTIES_BASE, prop);
